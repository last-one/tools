...
def parse_args():
    ...
    parser.add_argument('-b', '--blob', dest='blob', help='the name of layer extracred from', type=str)
...
def get_feature(net, transformer, args):
    ...
    for picture in os.listdir(args.input_path):
        ...
        for blob in blobs:
            feat = net.blobs[blob].data[0]
            feat_name = blob.split('_')[-1]
            feat_path = os.path.join(args.output_path, name + '_' + feat_name + '.npy')
